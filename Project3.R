...
# library(rgl) --sucessfully installed, but still having trouble loading
library(MLmetrics)
...
msedata <- data.frame(pred = zmb_adm2$lr_pop_sum, actual = zmb_adm2$pop19)
...
msedata <- data.frame(pred = zmb_adm2$rf_pop_sum, actual = zmb_adm2$pop19)
mean((msedata$actual - msedata$pred)^2)
